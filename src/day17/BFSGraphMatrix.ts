...
export default function bfs(graph: WeightedAdjacencyMatrix, source: number, needle: number): number[] | null {
    ...
    while (q.length) {
        ...
        for (let i=0; i<neighbors.length; ++i) {
            // no connection if value of neighbor is 0 in the weighted adjacency list
            if (seen[i] || neighbors[i] === 0) { continue; }
            // set previous and queue neighbor
            prev[i] = curr;
            q.enqueue(i);
        }
    }

    // start with needle and build the path to it backwards with the previous array
    const out: number[] = [];
